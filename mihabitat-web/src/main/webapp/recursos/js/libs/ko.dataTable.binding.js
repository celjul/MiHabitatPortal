	...
	'init' : function(element, valueAccessor, allBindingsAccessor, viewModel,
		...
		var options = {
				"language" : {
					...
					"emptyTable" : "No se ha encontrado ningun registro",
					"info" : "Mostrando del _START_ al _END_ de _TOTAL_ registros",
					"infoEmpty" : "No se ha encontrado ningun registro",
					"infoFiltered" : "(Se filtro de _MAX_ registros)",
...
//					"orderable" : false
//				} ],
//				"order" : [ [ columOrder, tipeOrder] ],
				"lengthMenu" : [ [ 10, 50, 100, -1 ], [ 10, 50, 100, "Todo" ]
				],
				"pageLength" : 10
		};

		// ** Initialise the DataTables options object with the data-bind
		// settings **
		// Clone the options object found in the data bindings. This object will
		// form the base for the DataTable initialisation object.
		if (binding.options)
		...
		if (binding.rowTemplate && binding.rowTemplate != '') {
			options.fnRowCallback = function(row, data, displayIndex,
					displayIndexFull) {
				// Render the row template for this row.
				// BUGFIX N Beemster
				// als dit gebruikt wordt, is de context niet beschikbaar in de
				// binding van de template
				// ko.renderTemplate(binding.rowTemplate, data, null, row,
				// "replaceChildren");
				return row;
			}
			...
			options.fnCreatedRow = function(nRow, aData, iDataIndex) {
				...
				var localContext = undefined;

				if (ko.bindingContext) {
					localContext = new ko.bindingContext(
							observableForThisRow, bindingContext);
				} else {
					localContext = bindingContext.createChildContext(
							observableForThisRow, bindingContext);
				}
				ko.renderTemplate(binding.rowTemplate, localContext, null,
						nRow, "replaceChildren");
			}
		...
		if (binding.dataSource) {
			...
			if (dataSource instanceof Array) {
				...
				options.aaData = ko.toJS(options.aaData);
				// If the data source is a knockout observable array...
				if (ko.isObservable(binding.dataSource)) {
					// Subscribe to the dataSource observable. This callback
					// will fire whenever items are added to
					...
					binding.dataSource.subscribe(function(newItems) {
						...
						ko.utils.arrayForEach(newItems, function(item) {
							unwrappedItems
									.push(ko.utils.unwrapObservable(item));
						});

						// Add the new data back into the data table.
						unwrappedItems = ko.toJS(unwrappedItems);
			...
			// If the dataSource was not a function that retrieves data, or a
			// javascript object array containing data.
			else {
				throw 'The dataSource defined must a javascript object array';
			}
		}

		// If no fnRowCallback has been registered in the DataTable's options,
		// then register the default fnRowCallback.
		// This default fnRowCallback function is called for every row in the
		// data source. The intention of this callback
		// is to build a table row that is bound it's associated record in the
		// data source via knockout js.
		if (!options.fnRowCallback) {
			options.fnRowCallback = function(row, srcData, displayIndex,
					displayIndexFull) {
				var columns = this.fnSettings().aoColumns

				// Empty the row that has been build by the DataTable of any
				// child elements.
				var destRow = $(row);
				destRow.empty();
		...
		// If no fnDrawCallback has been registered in the DataTable's options,
		// then register the default here.
		// This default callback is called every time the table is drawn (for
		// example, when the pagination is clicked).
		if (!options.fnDrawCallback) {
			options.fnDrawCallback = function() {
				// There are some assumptions here that need to be better
				// abstracted
				$(binding.expandIcon).click(
						function() {
							var theRow = $(this).parent().parent()[0]; 
							...
							if (tableId.fnIsOpen(theRow)) {
								$(this).removeClass(
										'icon-contract ' + binding.expandIcon);
								$(this).addClass(
										'icon-expand ' + binding.expandIcon);
		...
		// Tell knockout that the control rendered by this binding is capable of
		// managing the binding of it's descendent elements.
		// This is crutial, otherwise knockout will attempt to rebind elements
		// that have been printed by the row template.
		return {
		...
		ko.utils
				.arrayForEach(
						...
						function(item) {
							...
							if (item.name == "iDisplayStart")
							...
							else if (cog.string.startsWith(item.name,
									"bSearchable_"))
								...
								var sortOrder = ko.utils.arrayFilter(
										srcOptions, function(item) {
											return item.name == "sSortDir_"
													+ colIndex;
										});

								if (sortOrder.length
										&& sortOrder[0].value == "desc")
