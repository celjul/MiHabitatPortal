 ...
 */
@Controller
@RequestMapping(value = "administrador/reportes/pagos-contacto")
public class ReportePagosContactoController {

	@SuppressWarnings("unused")
	private static Logger LOG = LoggerFactory
			.getLogger(ReportePagosContactoController.class);
	...
	public String iniciar() {
		return "administrador/reportes/pagos-contacto";
	}

	@RequestMapping(method = RequestMethod.GET, value = "consultar")
	@ResponseBody
	public ReportePagos consultar(@RequestParam Short anio,
			HttpSession session) {
		return getReporte(
	...
	public ResponseEntity<byte[]> imprimir(@RequestParam String formato,
		...
		String filename = "pagos-contacto." + formato;
	...
	private byte[] getBytes(ReportePagos reporte, String formato) {
		...
		map.put("TITULO", "REPORTE DE PAGOS POR CONTACTO");
		...
		if (!CollectionUtils.isEmpty(pagos)) {
			for (Pago pago : pagos) {
				...
				if (calendar.get(Calendar.YEAR) == anio && !pago.getMetodoPago().getId().equals(configurationServiceImpl.getSaldoFavor().getId())) {
					...
					detalle.setId(pago.getContacto().getId());
					detalle.setNombre(pago.getContacto().getNombre() + " "
							+ pago.getContacto().getApellidoPaterno());
					switch (calendar.get(Calendar.MONTH)) {
					case 0:
						detalle.setEnero(pago.getMonto());
						break;
					case 1:
						detalle.setFebrero(pago.getMonto());
						break;
					case 2:
						detalle.setMarzo(pago.getMonto());
						break;
					case 3:
						detalle.setAbril(pago.getMonto());
						break;
					case 4:
						detalle.setMayo(pago.getMonto());
						break;
					case 5:
						detalle.setJunio(pago.getMonto());
						break;
					case 6:
						detalle.setJulio(pago.getMonto());
						break;
					case 7:
						detalle.setAgosto(pago.getMonto());
						break;
					case 8:
						detalle.setSeptiembre(pago.getMonto());
						break;
					case 9:
						detalle.setOctubre(pago.getMonto());
						break;
					case 10:
						detalle.setNoviembre(pago.getMonto());
						break;
					case 11:
						detalle.setDiciembre(pago.getMonto());
						break;
					}
